...
class DataCollector:
    ...
    def __init__(
        ...
        self.agent_lifetime_stats = [
            {"deaths": 0, "food_collected": 0} for _ in range(self.n_agents)
        ]

        # Log initial setup
        self.log(
            f"Simulation started at: {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
        )
    ...
    def generate_summary(self, elapsed_time):
        ...
        if self.agent_type == "curious_dqn":
            ...
            self._plot_intrinsic_vs_extrinsic_rewards()
    ...
    def _generate_summary_statistics(self, elapsed_time):
        ...
        avg_reward_per_agent = (
            self.cumulative_rewards / self.episodes_completed
            if self.episodes_completed > 0
            else 0
        )
        avg_reward_per_episode = (
            np.mean(self.avg_episode_rewards) if self.avg_episode_rewards else 0
        )
        avg_episode_length = (
            np.mean(self.episode_lengths) if self.episode_lengths else 0
        )
        median_episode_length = (
            np.median(self.episode_lengths) if self.episode_lengths else 0
        )
        ...
        avg_survival_rate = (
            np.mean(self.episode_survival_rates) if self.episode_survival_rates else 0
        )
        avg_deaths_per_episode = (
            np.mean(self.agent_deaths_per_episode)
            if self.agent_deaths_per_episode
            else 0
        )
        ...
        std_survival_rate = (
            np.std(self.episode_survival_rates) if self.episode_survival_rates else 0
        )
        ...
        self.log("=" * 50)
        self.log(
            f"Completed {self.episodes_completed} episodes with {self.n_agents} agents"
        )
        self.log(f"Agent type: {self.agent_type}")
        self.log(f"Total simulation time: {elapsed_time:.2f} seconds")
        self.log(
            f"Average time per episode: {elapsed_time/max(1, self.episodes_completed):.2f} seconds"
        )

        self.log("\nREWARD STATISTICS:")
        self.log(
            f"  Peak total episode reward: {self.peak_reward:.2f} (Episode {self.peak_reward_episode})"
        )
        self.log(
            f"  Average reward per episode: {avg_reward_per_episode:.2f} (±{std_reward:.2f})"
        )
        ...
        self.log("\nEPISODE STATISTICS:")
        self.log(
            f"  Episodes reaching maximum steps ({self.max_timesteps}): {self.episodes_reached_max_steps} ({self.episodes_reached_max_steps/max(1, self.episodes_completed)*100:.1f}%)"
        )
        self.log(
            f"  Average episode length: {avg_episode_length:.2f} steps (±{std_episode_length:.2f})"
        )
        self.log(f"  Median episode length: {median_episode_length:.1f} steps")
        self.log(
            f"  Min/Max episode length: {min_episode_length}/{max_episode_length} steps"
        )

        self.log("\nSURVIVAL STATISTICS:")
        self.log(
            f"  Average survival rate: {avg_survival_rate:.1f}% (±{std_survival_rate:.1f}%)"
        )
        self.log(f"  Average deaths per episode: {avg_deaths_per_episode:.2f}")
        for i in range(self.n_agents):
            death_rate = (
                self.agent_lifetime_stats[i]["deaths"]
                / max(1, self.episodes_completed)
                * 100
            )
            self.log(
                f"  Agent {i+1}: {self.agent_lifetime_stats[i]['deaths']} deaths ({death_rate:.1f}% of episodes)"
            )
        ...
        self.log(
            f"  Average episodic reward plot: {self.log_dir}/average_episodic_reward.png"
        )
        self.log(f"  Episode length plot: {self.log_dir}/episode_length_over_time.png")
        if self.agent_type == "curious_dqn":
            self.log(
                f"  Intrinsic rewards plot: {self.log_dir}/intrinsic_rewards_plot.png"
            )
        self.log("=" * 50)
    ...
    def _plot_rewards_large_dataset(self):
        ...
        rolling_avg_all = np.convolve(
            self.avg_episode_rewards, np.ones(window_size) / window_size, mode="valid"
        )
        plt.plot(
        ...
        plt.tight_layout()
        plt.savefig(
            f"{self.log_dir}/agent_rewards_per_episode.png", dpi=PLOT_CONFIG["dpi"]
        )
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(
            f"{self.log_dir}/average_episodic_reward.png", dpi=PLOT_CONFIG["dpi"]
        )
        plt.close()
    ...
    def _plot_intrinsic_rewards_large_dataset(self):
        ...
        for i, intrinsic_rewards in enumerate(self.episode_intrinsic_rewards):
            rolling_avg = np.convolve(
                intrinsic_rewards, np.ones(window_size) / window_size, mode="valid"
            )
            plt.plot(
        ...
        plt.savefig(
            f"{self.log_dir}/intrinsic_rewards_plot.png", dpi=PLOT_CONFIG["dpi"]
        )
        plt.close()
    ...
    def _plot_episode_lengths(self):
        ...
        plt.tight_layout()
        plt.savefig(
            f"{self.log_dir}/episode_length_over_time.png", dpi=PLOT_CONFIG["dpi"]
        )
        plt.close()
    ...
    def _plot_intrinsic_vs_extrinsic_rewards(self):
        ...
        if self.agent_type != "curious_dqn" or len(self.episode_rewards[0]) == 0:
            self.log(
                "Not a curious DQN agent or no episodes completed, skipping intrinsic vs extrinsic reward plot"
            )
            return
        ...
        if len(avg_extrinsic) > window_size:
            smoothed_extrinsic = np.convolve(
                avg_extrinsic, np.ones(window_size) / window_size, mode="valid"
            )
            smoothed_intrinsic = np.convolve(
                avg_intrinsic, np.ones(window_size) / window_size, mode="valid"
            )
        ...
        if len(normalised_ratios) > window_size:
            smoothed_ratio = np.convolve(
                normalised_ratios, np.ones(window_size) / window_size, mode="valid"
            )
            plt.plot(
        ...
        plt.axhline(
            ...
            linestyle="--",
            alpha=0.7,
            label="Balanced rewards (ratio=1)",
        )
        ...
        plt.tight_layout()
        plt.savefig(
            f"{self.log_dir}/intrinsic_vs_extrinsic_rewards.png", dpi=PLOT_CONFIG["dpi"]
        )
        plt.close()
