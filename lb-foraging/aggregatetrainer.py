...
def run_multiple_simulations(
    ...
    for agent_type in agent_types:
        ...
        for run in range(num_runs):
            ...
            # Extract metrics
            avg_reward = (
                np.mean(logger.avg_episode_rewards) if logger.avg_episode_rewards else 0
            )
            peak_reward = logger.peak_reward if hasattr(logger, "peak_reward") else 0
            avg_episode_length = (
                np.mean(logger.episode_lengths) if logger.episode_lengths else 0
            )
            survival_rate = (
                np.mean(logger.episode_survival_rates)
                if logger.episode_survival_rates
                else 0
            )
            episodes_max_steps = logger.episodes_reached_max_steps
            avg_deaths = (
                np.mean(logger.agent_deaths_per_episode)
                if logger.agent_deaths_per_episode
                else 0
            )
...
def plot_learning_curves(results_dir, agent_types, num_runs):
    ...
    for i, agent_type in enumerate(agent_types):
        if agent_type in all_agent_data:
            ...
            ax.plot(
                episodes, mean_smooth, color=colors.get(agent_type, "blue"), linewidth=2
            )
    ...
    plt.savefig(
        f"{results_dir}/smooth_learning_curves.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def generate_comparison_plots(results, agent_types, results_dir):
    ...
    for metric_key, metric_name in metrics:
        ...
        plt.savefig(
            f"{results_dir}/{metric_key}_comparison.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
