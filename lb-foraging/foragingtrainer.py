...
class ForagingTrainer:
    ...
    def _create_agents(self):
        ...
        if self.agent_type == "random":
            ...
            self.logger.log(
                f"Unknown agent type: {self.agent_type}, defaulting to Curious DQN"
            )
            return [
                CuriosityDrivenDQNAgent(agent_params=self.agent_params)
                for _ in range(self.n_agents)
    ...
    def train(self):
        ...
        for episode in range(self.num_episodes):
            ...
            self.logger.log(
                f"Initial energy: {[agent.energy for agent in self.agents]}"
            )
            ...
            for step in range(self.max_steps):
                ...
                for i, (agent, obs) in enumerate(zip(self.agents, obss)):
                    if agent.energy > 0:
                        ...
                        if not agents_died_this_episode[i] and agent.energy <= 0:
                            ...
                            self.logger.log(
                                f"  Step {step + 1}: Agent {i+1} died (energy depleted)"
                            )
                ...
                if not valid_actions:
                    self.logger.log(
                        f"Episode {episode + 1} ended early at step {step + 1}: All agents are dead."
                    )
                    break
                ...
                for i, (agent, rew) in enumerate(zip(self.agents, rews)):
                    ...
                    if self.agent_type == "curious_dqn":
                        ...
                        try:
                            ...
                            if (
                                hasattr(agent, "__dict__")
                                and "intrinsic_rewards" in agent.__dict__
                            ):
                ...
                if self.display_info:
                    self.logger.log(
                        f"  Step {step + 1}: Rewards {rews}, Energy {[agent.energy for agent in self.agents]}, Actions {actions}, Positions {[player.position for player in self.env.unwrapped.players]}"
                    )
                ...
                if done or trunc:
                    self.logger.log(
                        f"Episode {episode + 1} terminated at step {step + 1} with done={done}, truncated={trunc}"
                    )
                    break
            ...
            if reached_max_steps:
                ...
                self.logger.log(
                    f"Episode {episode + 1} reached maximum {self.max_steps} steps"
                )
