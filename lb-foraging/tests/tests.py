...
class TestSustainableForagingEnvironment(unittest.TestCase):
    ...
    def setUp(self):
        ...
        if (
            not hasattr(self.env.unwrapped.players[0], "controller")
            or self.env.unwrapped.players[0].controller is None
        ):
            ...
            self.env.unwrapped.players[0].controller.position = (
                self.env.unwrapped.players[0].position
            )
        else:
